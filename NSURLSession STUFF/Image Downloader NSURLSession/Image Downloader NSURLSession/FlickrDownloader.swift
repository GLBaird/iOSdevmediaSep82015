        ...
        if let url = getRequestURL(searchText) {
            ...
            let task = session.dataTaskWithURL(url, completionHandler: {
                ...
                if error == nil && jsonData != nil {
                    ...
                    let error = NSError(domain: "FlickrDownloader", code: 123, userInfo: [
                        "localizedDescription": "Failed to download and parse Flickr Files",
                        "localizedRecoverySuggestion": "Check Flickr API Documentation agains this class"
                    ])
                    completion?(images:nil, error:error)
                    self.delegate?.flickrDownloader(self, receivedError: error)
                } else {
